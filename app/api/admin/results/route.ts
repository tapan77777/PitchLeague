...
// POST /api/admin/results — body: { match_id, score_a, score_b, league_id }
export async function POST(req: NextRequest) {
  const { match_id, score_a, score_b, league_id } = await req.json()

  if (!match_id || !league_id || score_a == null || score_b == null) {
  ...
  await supabase.rpc('update_league_ranks', { p_league_id: league_id })
